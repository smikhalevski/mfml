...
import { MessageNode, renderText } from '../main/index.js';
...
test('renders message as plain text', () => {
  ...
  expect(renderText({ message: () => messageNode, locale: 'en', values: { bbb: 'zzz' } })).toBe('aaa zzz');
});

test('does not throw if values were not provided', () => {
  const messageNode = parseMessage('en', 'aaa <xxx>{bbb}</xxx>', { tokenizer });

  expect(renderText({ message: () => messageNode, locale: 'en' })).toBe('aaa ');
});
...
test('renders plural', () => {
  ...
  expect(renderText({ message: () => messageNode, locale: 'en', values: { bbb: 2 } })).toBe('aaazzz');
});
