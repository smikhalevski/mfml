...
test('renders date', () => {
  expect(mockMessageRenderer.formatArgument('ru', 0, 'date', undefined)).toBe('01.01.1970');
});
...
test('renders time', () => {
  expect(mockMessageRenderer.formatArgument('ru', 0, 'time', undefined)).toBe('01.01.1970');
});
...
test('renders number', () => {
  expect(mockMessageRenderer.formatArgument('ru', 111.222, 'number', undefined)).toBe('111,222');
});
...
test('renders null and undefined as an empty string', () => {
  expect(mockMessageRenderer.formatArgument('ru', null, 'number', 'xxx')).toBe('');
  expect(mockMessageRenderer.formatArgument('ru', undefined, 'number', 'xxx')).toBe('');
});

test('renders unknown argument types as a string', () => {
  expect(mockMessageRenderer.formatArgument('ru', { toString: () => 'zzz' }, 'vvv', 'xxx')).toBe('zzz');
  expect(mockMessageRenderer.formatArgument('ru', 111.222, 'vvv', 'xxx')).toBe('111.222');
});
